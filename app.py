...
def show_todo_list():
    ...
    if request.method == 'GET':
        ...
        if current_user:
            ...
            flag = 0
        print(flag)
        return render_template('index.html', todolists=todolists,flag = flag,form=form)
    else:
        if form.validate_on_submit():
            print(form.title.data+'-----'+form.status.data+"----"+form.time.data)
            todolist = TodoList(current_user.id, form.title.data, form.status.data,form.time.data)
            # print(todolist)
...
def change_todo_list(id):
    if request.method == 'GET':
        ...
        if form.validate_on_submit():
            ...
            todolist.create_time = form.time.data
            db.session.commit()
